...
async def login_via_google(request: Request):
    ...
    # Assurer que l'URL est absolue (important pour OAuth)
    if not str(redirect_uri).startswith('http'):
        # Construire l'URL absolue (adapter si derrière un proxy complexe)
        base_url = str(request.base_url).rstrip('/')
        redirect_uri = base_url + str(redirect_uri)
